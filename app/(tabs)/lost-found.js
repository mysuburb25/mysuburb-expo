...
export default function LostFoundScreen() {
  ...
  useEffect(() => { if (profile?.suburb) fetchItems(); }, [profile, filter]);
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      if (filter !== 'all') data = data.filter(p => p.lostFoundType === filter);
      setItems(data);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>{profile?.suburb}</Text>
      </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.card} onPress={() => router.push(`/post/${item.id}`)}>
              <View style={[styles.typeBadge, { backgroundColor: item.lostFoundType === 'lost' ? Colors.terracotta : Colors.brandGreen }]}>
                <Text style={styles.typeText}>{item.lostFoundType?.toUpperCase() || 'LOST/FOUND'}</Text>
              </View>
              <Text style={styles.cardContent} numberOfLines={3}>{item.content}</Text>
              <Text style={styles.cardAuthor}>by {item.authorName}</Text>
            </TouchableOpacity>
          )}
          ListEmptyComponent={<View style={styles.empty}><Ionicons name="search-outline" size={48} color={Colors.lightGrey} /><Text style={styles.emptyText}>No lost & found posts</Text></View>}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  typeText: { fontSize: 11, fontWeight: '700', color: Colors.white },
